...
try {
  porteDejaOuverte = sessionStorage.getItem('ma2m_porte_ouverte');
} catch (e) {}
...
document.getElementById('entrer-btn').addEventListener('click', async () => {
  ...
  try {
    sessionStorage.setItem('ma2m_porte_ouverte', '1');
  } catch (e) {}
...
(function () {
  ...
  try {
    dejaVue = sessionStorage.getItem('ma2m_intro_vue');
  } catch (e) {}
  ...
  function terminerIntro() {
    ...
    try {
      sessionStorage.setItem('ma2m_intro_vue', '1');
    } catch (e) {}
  ...
  document
    .getElementById('splash-passer')
    .addEventListener('click', terminerIntro);

  // Filet de sécurité : si le petit bouton "Passer l'intro" n'est pas visible ou
  // pas atteignable sur certains téléphones, un appui n'importe où sur l'écran
  // d'intro permet quand même de passer directement au site.
